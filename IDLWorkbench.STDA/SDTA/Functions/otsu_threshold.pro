...
;-
   ...
   FredmanDiaconis = 0.5*IQR(image)*(N_ELEMENTS(image)^(-1.0/3.0))
   scott = 3.49*STDDEV(image)*(N_ELEMENTS(image)^(-1.0/3))
   
   h = HISTOGRAM( image - minimumDC, OMIN=minDC, OMAX=maxDC, BINSIZE=FredmanDiaconis, locations=x )
