...
;################################
FUNCTION GET_NEIGH_PARS_MULTI, Image, winSizes

  Dim = SIZE(Image,/DIMENSION)
  NB = Dim[0]   &   NC = Dim[1]   &   NL = Dim[2]

  VRegs = PTRARR(N_ELEMENTS(winSizes),NC,NL)

  FOR k = 0, N_ELEMENTS(winSizes)-1 DO BEGIN
    FOR i = 0L, NC-1 DO BEGIN
      FOR j = 0L, NL-1 DO BEGIN

        Samples = GET_QUICK_NEIGH_VALS(i,j,winSizes[k],winSizes[k],NB,NC,NL,Image)
        ...
        ;VRegs[k,i,j] = PTR_NEW({Mu: MeanVec, Sigma: SigMatrix, InvSigma: InvSigma})
        VRegs[k,i,j] = PTR_NEW({Mu: FLOAT(MeanVec), Sigma: FLOAT(SigMatrix)}) ;, InvSigma: FLOAT(InvSigma)})
      ENDFOR
    ENDFOR
  ENDFOR
